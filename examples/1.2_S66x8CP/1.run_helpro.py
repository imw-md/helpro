...
from helpro.molpro.counterpoise import counterpoise, tag_multimer
...
def run_dimers(miw: MolproInputWriter) -> None:
    ...
    for structure in structures:
        print(structure)
        monomers = [ase.io.read(parent / f"{structure}_{_}.xyz") for _ in (1, 2)]
        for dimer in [0.90, 0.95, 1.00, 1.05, 1.10, 1.25, 1.50, 2.00]:
            ...
            with cd(directory):
                ...
                multimer = ase.io.read(xyz)
                tags = tag_multimer(multimer, monomers)
                directories = counterpoise(miw, tags)
                for directory_counterpoise in directories:
                    shutil.copy2(xyz, directory_counterpoise / "initial.xyz")
